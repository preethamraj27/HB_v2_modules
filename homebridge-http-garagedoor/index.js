...
function RegexMapper(parameters) {
	...
	console.log("parameters.regexp:" + parameters.regexp)
	console.log("parameters.capture:" + parameters.capture)
	self.map = function(value) {
		console.log("value:" + value)
		console.log("self.regexp:" + self.regexp)
...
function HttpGarageDoorAccessory(log, config) {
	...
	//Setup Cache's to update the UI asap
	self.currentDoorStateCache = new Cache(config.currentStateCache, 90 *1000);
	self.targetDoorStateCache  = new Cache(config.targetStateCache, 90 *1000);
    ...
    api.on('didFinishLaunching', function() {
        ...
        if (api.notificationRegistration && typeof api.notificationRegistration === "function") {
            try {
                api.notificationRegistration(config.notificationID, this.handleNotification.bind(this), config.notificationPassword);
            } catch (error) {
                // notificationID is already taken
                this.log("Could not register notification handler. ID '" + notificationID + "' is already taken!")
            }
        }
    }.bind(this));
...
HttpGarageDoorAccessory.prototype.init = function() {
	...
	if (self.polling) {
		...
		emitter.on("longpoll", function (state) {
			...
			self.garagedoorService.getCharacteristic(Characteristic.CurrentDoorState).setValue(state);
		});
...
/**
Function to work with http-notification server
...
*/
HttpGarageDoorAccessory.prototype.handleNotification =  function(body) {
	this.log("Handler for garage hit !"+body.characteristic+body.value)
   ...
   /* if (!this.homebridgeService.testCharacteristic(body.characteristic)) {
    ...
    self.log("Received a notification, Characteristic: "+body.characteristic+" & Value: " +state)

    if (body.characteristic === "CurrentDoorState"){// && this.polling) 
        //this.emitter.clear();
		self.currentDoorStateCache.queried();
		self.targetDoorStateCache.queried();
        self.garagedoorService
			.setCharacteristic(Characteristic.CurrentDoorState, state);
		self.previousCurrentState = self.garagedoorService.getCharacteristic(Characteristic.CurrentDoorState).value;
		self.previousTargetState = self.garagedoorService.getCharacteristic(Characteristic.TargetDoorState).value;
        if (state == 2) { // if opening(2) , then target state is Open(0)
			self.garagedoorService
				.setCharacteristic(Characteristic.TargetDoorState, Characteristic.TargetDoorState.OPEN);
		}
		else if (state == 3) {// if closing(3) , then target state is close(1)
				self.garagedoorService
		...
		else{ //Anything else & its probably something wrong, so say OPEN(0)
			self.garagedoorService
			.setCharacteristic(Characteristic.TargetDoorState, Characteristic.TargetDoorState.OPEN);
		}

	}
	else if (body.characteristic === "TargetDoorState"){
		self.garagedoorService
			.getCharacteristic(Characteristic.TargetDoorState)
			.setValue(state);
...
HttpGarageDoorAccessory.prototype.getState = function(url, body, callback) {
	...
	this.httpRequest(url, body, function(error, response, responseBody) {
		if (error) {
			...
			var state = responseBody.replace(/"/g, '\\"');
			this.debugLog("Read state resp:" + state);
...
HttpGarageDoorAccessory.prototype.getCurrentState = function(callback) {
	var self = this;
	if(!self.currentDoorStateCache.shouldQuery()){
		var state = self.garagedoorService.getCharacteristic(Characteristic.CurrentDoorState).value
		self.debugLog("Getting currentDoorState Cache %s",state);
		callback(null,state);
		return;
	}
	self.debugLog("Cache seems to be old.. Getting current state");
...
HttpGarageDoorAccessory.prototype.getTargetState =  function(callback) {
	var self = this;
	if(!self.targetDoorStateCache.shouldQuery()){
		var state = self.garagedoorService.getCharacteristic(Characteristic.TargetDoorState).value
		self.debugLog("Getting targetDoorState Cache %s",state);
		callback(null,state);
		return;
	}
	self.debugLog("Cache seems to be old..Getting target state");
